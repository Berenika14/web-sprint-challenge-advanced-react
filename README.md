...
- A class component must include the extends React.Component statement. This statement creates an inheritance to react and gives the component access to React.Component functions.The class component also requires a render method this method return HTML.
  Class components are used also to handle state management and wrap child components.
- A functional component is just plain JavaScript function that takes props as an argument and return a React element. Functional components call functions from parent components to handle state updates.Functional component can we be written using less code.

2. When does a componentWillMount function be called? What about a componentDidUpdate?

- componentWillMount function is called before the render method is executed.
- componentDidUpdate function is called when a component got updated this can happen when you add props or when an internal state has has been changed.

3. Define stateful logic.

\*Stateful logic is a logic code build into a components. It can be a function that handles a click event or a change on state.

4. What are the three step of creating a successful test? What is done in each phase?

\*Arrange:inputs and targets,check to see if the render method renders a React element into a virtual DOM.

\*Act: target the behavior, it should cover the main thing to be tested. This could be calling a function or method, calling a REST API, or interacting with a web page

\*Assert: expected outcomes, it should give a sort of response. For example check ot see if the input is a string value.In this step we an determine if the test passes or fails
